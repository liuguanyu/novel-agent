...
- [ ] 1.1 [Core] 定义 `WorkflowKind`、`WorkflowStatus`、`StageStatus`、独立 `StageImpactStatus`/`impactStatus`、`StageActor`、`WorkflowScope`、`WorkflowRef`、`WorkflowInstance`、`WorkflowStageInstance` 与 artifact/blocking reason 精确类型，保持无 Electron/React/LangGraph 依赖
...
- [ ] 1.6 [Core/Test] 覆盖非法跳过、失败不推进、重试保留 run、重复命令幂等、问题只有复检后 resolved、resolved 后复发 reopen，及 discovery/audit/transition/resolution history 等状态机单元测试
